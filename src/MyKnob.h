...
#define COMBO_MAX_ITEMS 5  // recognize combo sequences of up to 5 presses
  ...
  void checkButton(int *_aiIndex) {
    ...
    // if (millis() - holdingSince > sleepPress && lastButtonState == 0 &&
    // buttonState == 0) {
    //   offMode = !offMode;
    //   lastButtonState = 1;  // virtual relase of button
    //   holdingSince = 0;
    //   Serial.print("\n\n ====== intercepted VERY LONG PRESS! fliping
    //   offMode...  ======");

    // } else if (lastButtonState == 0 && buttonState == 1) { // after first
    // frame of press interval lastButtonState == 0

    if (lastButtonState == 0 &&
        buttonState ==
            1) {  // after first frame of press interval lastButtonState == 0
      // A press or hold was finished
      unsigned long currentPressTime = millis();
      unsigned long holdTime = currentPressTime - holdingSince;
      unsigned long difference =
          currentPressTime - lastPressTime -
          holdTime;  // compensate for duration of push interval
      ...
      //   BUG can't use difference for first press in combo since it could be
      //   really large and therefore always indicate lastPressWasLong
      //
      ...
      // momentary press typically takes 90-130ms, but possible to get as fast
      // as 30ms
      if (thisPressIsShort) {
        if (firstPressInCmdMode || (cmdMode && stillTimeToCombo))
          addToCombo('.');
  ...
  /////////////////////////////////////////////////////////
  // hashmap implementation instead of switch statement (allows string->fx()
  // mapping)
  //
  ...
  //     unordered_map<string, funcPointer>::const_iterator got = modeMap.find
  //     (pat); if( got == stringFunctionMap.end() ) {
  //         cout << "Error: patternDispatcher() unable to find pattern " << pat
  //         << " in stringFunctionMap\n";
  //     } else {
